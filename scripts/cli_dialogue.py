...
def init_model(model_path, config_path, model_config_path):
    ...
    # 加载配置
    config = load_config(os.path.join('..', config_path))
    model_config = load_config(os.path.join('..', model_config_path))
    ...
    # 加载模型权重
    model, _, _, _ = load_model(model, None, os.path.join('..', model_path))
    model.eval()
...
def cli_dialogue(model_path, config_path, model_config_path):
    init_model(model_path, config_path, model_config_path)
    ...
    while True:
        ...
        print(f"回复: {prediction}")

if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='Transformer Model CLI Dialogue')
    ...
    cli_dialogue(args.model_path, args.config_path, args.model_config_path)